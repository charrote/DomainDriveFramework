<template>
  <div class="page-container">
    ...
    <div class="page-header">
      <h1 class="page-title">工单 BOM 明细</h1>
...
import { getWOBOMItemList, deleteWOBOMItem } from '@/api/wobom-item'
...
async function fetchData() {
  try {
    const res = await getWOBOMItemList({
      page: page.value,
    ...
    console.error('Failed to fetch WOBOMItem:', err)
  }
}
...
function handleCreate() {
  router.push({ name: 'wobom-item-create' })
}

function handleEdit(id) {
  router.push({ name: 'wobom-item-edit', params: { id } })
}

async function handleDelete(id) {
  if (!confirm('确认删除？')) return
  try {
    await deleteWOBOMItem(id)
    fetchData()
  } catch (err) {
