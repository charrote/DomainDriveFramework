      ...
      <table class="ddf-table">
        ...
        <tbody>
          <tr v-for="row in dataList" :key="row.id">
            ...
            <td>
              <DdfTag color="primary" size="sm">{{ row.entity_type }}</DdfTag>
            </td>
            ...
            <td>
              <DdfTag color="primary" size="sm">{{ row.seq_strategy }}</DdfTag>
            </td>
...
import { getCodingRuleList, deleteCodingRule } from '@/api/coding-rule'
...
async function fetchData() {
  try {
    const res = await getCodingRuleList({
      page: page.value,
    ...
    console.error('Failed to fetch CodingRule:', err)
  }
}
...
function handleCreate() {
  router.push({ name: 'coding-rule-create' })
}

function handleEdit(id) {
  router.push({ name: 'coding-rule-edit', params: { id } })
}

async function handleDelete(id) {
  if (!confirm('确认删除？')) return
  try {
    await deleteCodingRule(id)
    fetchData()
  } catch (err) {
