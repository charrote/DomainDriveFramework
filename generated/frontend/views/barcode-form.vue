<template>
  <div class="page-container">
    ...
    <div class="page-header">
      <h1 class="page-title">{{ isEdit ? '编辑' : '新建' }}条码</h1>
    ...
    <div class="form-wrapper">
      <div class="form-grid">
        ...
        <div class="form-group" :class="'string'">
          ...
          <input type="text" v-model="form.barcode" class="form-input" />
        </div>
        ...
        <div class="form-group" :class="'string'">
          ...
          <input type="text" v-model="form.batch_no" class="form-input" />
        </div>
        ...
        <div class="form-group" :class="'date'">
          ...
          <input type="date" v-model="form.production_date" class="form-input" />
        </div>
        ...
        <div class="form-group" :class="'relation'">
          ...
          <input type="number" v-model.number="form.container_id" class="form-input" placeholder="Container" />
        </div>
...
import { getBarcode, createBarcode, updateBarcode } from '@/api/barcode'
...
async function fetchDetail() {
  try {
    const res = await getBarcode(route.params.id)
    form.value = res
  } catch (err) {
...
async function handleSubmit() {
  ...
  try {
    if (isEdit.value) {
      await updateBarcode(route.params.id, form.value)
    } else {
      await createBarcode(form.value)
    }
    router.push({ name: 'barcode-list' })
  } catch (err) {
