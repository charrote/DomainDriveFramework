<template>
  <div class="page-container">
    ...
    <div class="page-header">
      <h1 class="page-title">{{ isEdit ? '编辑' : '新建' }}工厂</h1>
    ...
    <div class="form-wrapper">
      <div class="form-grid">
        ...
        <div class="form-group" :class="'string'">
          ...
          <input type="text" v-model="form.factory_code" class="form-input" />
        </div>
        ...
        <div class="form-group" :class="'string'">
          ...
          <input type="text" v-model="form.factory_name" class="form-input" />
        </div>
...
import { getFactory, createFactory, updateFactory } from '@/api/factory'
...
async function fetchDetail() {
  try {
    const res = await getFactory(route.params.id)
    form.value = res
  } catch (err) {
...
async function handleSubmit() {
  ...
  try {
    if (isEdit.value) {
      await updateFactory(route.params.id, form.value)
    } else {
      await createFactory(form.value)
    }
    router.push({ name: 'factory-list' })
  } catch (err) {
