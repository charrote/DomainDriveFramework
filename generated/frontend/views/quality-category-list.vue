...
import { getQualityCategoryList, deleteQualityCategory } from '@/api/quality-category'
...
async function fetchData() {
  try {
    const res = await getQualityCategoryList({
      page: page.value,
    ...
    console.error('Failed to fetch QualityCategory:', err)
  }
}
...
function handleCreate() {
  router.push({ name: 'quality-category-create' })
}

function handleEdit(id) {
  router.push({ name: 'quality-category-edit', params: { id } })
}

async function handleDelete(id) {
  if (!confirm('确认删除？')) return
  try {
    await deleteQualityCategory(id)
    fetchData()
  } catch (err) {
