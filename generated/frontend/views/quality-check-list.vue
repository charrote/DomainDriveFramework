      ...
      <table class="ddf-table">
        ...
        <tbody>
          <tr v-for="row in dataList" :key="row.id">
            ...
            <td>
              <DdfTag color="primary" size="sm">{{ row.check_type }}</DdfTag>
            </td>
...
import { getQualityCheckList, deleteQualityCheck } from '@/api/quality-check'
...
async function fetchData() {
  try {
    const res = await getQualityCheckList({
      page: page.value,
    ...
    console.error('Failed to fetch QualityCheck:', err)
  }
}
...
function handleCreate() {
  router.push({ name: 'quality-check-create' })
}

function handleEdit(id) {
  router.push({ name: 'quality-check-edit', params: { id } })
}
