...
// 字段白名单（排除审计字段）
const FIELDS = ['factory_code', 'factory_name', 'status'];

// 可排序字段列表（由 DDF Engine 在生成期计算）
const SORTABLE_FIELDS = ['id', 'factory_code', 'factory_name', 'created_at'];

// 可搜索字段列表（由 DDF Engine 在生成期计算）
const SEARCHABLE_FIELDS = ['factory_code', 'factory_name', 'status'];
...
router.get('/', async (req, res) => {
  try {
    ...
    const [data, total] = await Promise.all([
      prisma.factory.findMany({ where, skip, take, orderBy }),
