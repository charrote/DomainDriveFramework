<template>
  <div class="page-container">
    ...
    <div class="page-header">
      <h1 class="page-title">{{ isEdit ? '编辑' : '新建' }}工位</h1>
    ...
    <div class="form-wrapper">
      <div class="form-grid">
        ...
        <div class="form-group" :class="'string'">
          ...
          <input type="text" v-model="form.station_code" class="form-input" />
        </div>
        ...
        <div class="form-group" :class="'string'">
          ...
          <input type="text" v-model="form.station_name" class="form-input" />
        </div>
...
import { getWorkstation, createWorkstation, updateWorkstation } from '@/api/workstation'
...
async function fetchDetail() {
  try {
    const res = await getWorkstation(route.params.id)
    form.value = res
  } catch (err) {
...
async function handleSubmit() {
  ...
  try {
    if (isEdit.value) {
      await updateWorkstation(route.params.id, form.value)
    } else {
      await createWorkstation(form.value)
    }
    router.push({ name: 'workstation-list' })
  } catch (err) {
