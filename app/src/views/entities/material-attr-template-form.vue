<template>
  <div class="page-container">
    ...
    <div class="page-header">
      <h1 class="page-title">{{ isEdit ? '编辑' : '新建' }}物料属性模板</h1>
    ...
    <div class="form-wrapper">
      <div class="form-grid">
        ...
        <div class="form-group" :class="'string'">
          ...
          <input type="text" v-model="form.attr_code" class="form-input" />
        </div>
        ...
        <div class="form-group" :class="'string'">
          ...
          <input type="text" v-model="form.attr_name" class="form-input" />
        </div>
        ...
        <div class="form-group" :class="'enum'">
          ...
          <select v-model="form.attr_type" class="form-input">
...
import { getMaterialAttrTemplate, createMaterialAttrTemplate, updateMaterialAttrTemplate } from '@/api/material-attr-template'
...
async function fetchDetail() {
  try {
    const res = await getMaterialAttrTemplate(route.params.id)
    form.value = res
  } catch (err) {
...
async function handleSubmit() {
  ...
  try {
    if (isEdit.value) {
      await updateMaterialAttrTemplate(route.params.id, form.value)
    } else {
      await createMaterialAttrTemplate(form.value)
    }
    router.push({ name: 'material-attr-template-list' })
  } catch (err) {
