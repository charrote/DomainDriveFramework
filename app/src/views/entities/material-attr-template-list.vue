      ...
      <table class="ddf-table">
        ...
        <tbody>
          <tr v-for="row in dataList" :key="row.id">
            ...
            <td>
              <DdfTag color="primary" size="sm">{{ row.attr_type }}</DdfTag>
            </td>
            ...
            <td>
              <DdfStatusIndicator :status="row.required ? 'active' : 'inactive'" />
            </td>
...
import { getMaterialAttrTemplateList, deleteMaterialAttrTemplate } from '@/api/material-attr-template'
...
async function fetchData() {
  try {
    const res = await getMaterialAttrTemplateList({
      page: page.value,
    ...
    console.error('Failed to fetch MaterialAttrTemplate:', err)
  }
}
...
function handleCreate() {
  router.push({ name: 'material-attr-template-create' })
}

function handleEdit(id) {
  router.push({ name: 'material-attr-template-edit', params: { id } })
}

async function handleDelete(id) {
  if (!confirm('确认删除？')) return
  try {
    await deleteMaterialAttrTemplate(id)
    fetchData()
  } catch (err) {
