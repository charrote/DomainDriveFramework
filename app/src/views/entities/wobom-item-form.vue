<template>
  <div class="page-container">
    ...
    <div class="page-header">
      <h1 class="page-title">{{ isEdit ? '编辑' : '新建' }}工单 BOM 明细</h1>
    ...
    <div class="form-wrapper">
      <div class="form-grid">
        ...
        <div class="form-group" :class="'float'">
          ...
          <input type="number" v-model.number="form.required_qty" class="form-input" step="0.01" />
        </div>
        ...
        <div class="form-group" :class="'float'">
          ...
          <input type="number" v-model.number="form.consumed_qty" class="form-input" step="0.01" />
        </div>
...
import { getWOBOMItem, createWOBOMItem, updateWOBOMItem } from '@/api/wobom-item'
...
function initForm() {
  ...
  data['required_qty'] = null
  
  
  
  data['consumed_qty'] = null
...
async function fetchDetail() {
  try {
    const res = await getWOBOMItem(route.params.id)
    form.value = res
  } catch (err) {
...
async function handleSubmit() {
  ...
  try {
    if (isEdit.value) {
      await updateWOBOMItem(route.params.id, form.value)
    } else {
      await createWOBOMItem(form.value)
    }
    router.push({ name: 'wobom-item-list' })
  } catch (err) {
