<template>
  <div class="page-container">
    ...
    <div class="page-header">
      <h1 class="page-title">{{ isEdit ? '编辑' : '新建' }}工艺路线</h1>
    ...
    <div class="form-wrapper">
      <div class="form-grid">
        ...
        <div class="form-group" :class="'string'">
          ...
          <input type="text" v-model="form.route_code" class="form-input" />
        </div>
        ...
        <div class="form-group" :class="'integer'">
          ...
          <input type="number" v-model.number="form.total_standard_time" class="form-input" step="1" />
        </div>
...
import { getProcessRoute, createProcessRoute, updateProcessRoute } from '@/api/process-route'
...
async function fetchDetail() {
  try {
    const res = await getProcessRoute(route.params.id)
    form.value = res
  } catch (err) {
...
async function handleSubmit() {
  ...
  try {
    if (isEdit.value) {
      await updateProcessRoute(route.params.id, form.value)
    } else {
      await createProcessRoute(form.value)
    }
    router.push({ name: 'process-route-list' })
  } catch (err) {
