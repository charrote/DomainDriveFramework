      ...
      <table class="ddf-table">
        ...
        <tbody>
          ...
          <tr v-if="!dataList.length">
            <td :colspan="4" class="td-empty">暂无数据</td>
          </tr>
        </tbody>
      </table>
    </div>
...
import { getMaterialAttrValueList, deleteMaterialAttrValue } from '@/api/material-attr-value'
...
async function fetchData() {
  try {
    const res = await getMaterialAttrValueList({
      page: page.value,
    ...
    console.error('Failed to fetch MaterialAttrValue:', err)
  }
}
...
function handleCreate() {
  router.push({ name: 'material-attr-value-create' })
}

function handleEdit(id) {
  router.push({ name: 'material-attr-value-edit', params: { id } })
}

async function handleDelete(id) {
  if (!confirm('确认删除？')) return
  try {
    await deleteMaterialAttrValue(id)
    fetchData()
  } catch (err) {
