<template>
  <div class="page-container">
    ...
    <div class="page-header">
      <h1 class="page-title">{{ isEdit ? '编辑' : '新建' }}生产记录</h1>
    ...
    <div class="form-wrapper">
      <div class="form-grid">
        ...
        <div class="form-group" :class="'string'">
          ...
          <input type="text" v-model="form.record_no" class="form-input" />
        </div>
        ...
        <div class="form-group" :class="'integer'">
          ...
          <input type="number" v-model.number="form.output_quantity" class="form-input" step="1" />
        </div>
...
import { getProductionRecord, createProductionRecord, updateProductionRecord } from '@/api/production-record'
...
async function fetchDetail() {
  try {
    const res = await getProductionRecord(route.params.id)
    form.value = res
  } catch (err) {
...
async function handleSubmit() {
  ...
  try {
    if (isEdit.value) {
      await updateProductionRecord(route.params.id, form.value)
    } else {
      await createProductionRecord(form.value)
    }
    router.push({ name: 'production-record-list' })
  } catch (err) {
