      ...
      <table class="ddf-table">
        ...
        <tbody>
          <tr v-for="row in dataList" :key="row.id">
            ...
            <td>
              <DdfTag color="primary" size="sm">{{ row.data_type }}</DdfTag>
            </td>
...
import { getProcessParamList, deleteProcessParam } from '@/api/process-param'
...
async function fetchData() {
  try {
    const res = await getProcessParamList({
      page: page.value,
    ...
    console.error('Failed to fetch ProcessParam:', err)
  }
}
...
function handleCreate() {
  router.push({ name: 'process-param-create' })
}

function handleEdit(id) {
  router.push({ name: 'process-param-edit', params: { id } })
}

async function handleDelete(id) {
  if (!confirm('确认删除？')) return
  try {
    await deleteProcessParam(id)
    fetchData()
  } catch (err) {
