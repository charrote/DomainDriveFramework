      ...
      <table class="ddf-table">
        ...
        <tbody>
          <tr v-for="row in dataList" :key="row.id">
            ...
            <td>
              <DdfStatusIndicator :status="row.certified ? 'active' : 'inactive'" />
            </td>
            
            <td>
              {{ row.expire_date ? new Date(row.expire_date).toLocaleString() : '-' }}
            </td>
...
import { getOperatorSkillList, deleteOperatorSkill } from '@/api/operator-skill'
...
async function fetchData() {
  try {
    const res = await getOperatorSkillList({
      page: page.value,
    ...
    console.error('Failed to fetch OperatorSkill:', err)
  }
}
...
function handleCreate() {
  router.push({ name: 'operator-skill-create' })
}

function handleEdit(id) {
  router.push({ name: 'operator-skill-edit', params: { id } })
}

async function handleDelete(id) {
  if (!confirm('确认删除？')) return
  try {
    await deleteOperatorSkill(id)
    fetchData()
  } catch (err) {
